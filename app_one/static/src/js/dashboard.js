...
const todayStr = formatDate(today);
    ...
    const savedSearch = sessionStorage.getItem("hr_dashboard_search") || "";
    const savedActiveFilters = JSON.parse(sessionStorage.getItem("hr_dashboard_active_filters")) || { active_emp: false, archived_emp: false };
    const savedGroupBy = JSON.parse(sessionStorage.getItem("hr_dashboard_group_by")) || [];
    ...
    this.state = useState({
        ...
        search_query: savedSearch || (defaultFav ? defaultFav.search_query : ''),
        active_filters: savedSearch ? savedActiveFilters : (defaultFav ? { ...defaultFav.active_filters } : savedActiveFilters),
        group_by_list: savedSearch ? savedGroupBy : (defaultFav ? [...defaultFav.group_by_list] : savedGroupBy),
...
async onSearchKeyUp(ev) {
    if (ev.key === 'Enter' && ev.target.value.trim() !== '') {
        ...
        sessionStorage.setItem("hr_dashboard_search", this.state.search_query);
        ev.target.value = '';
        await this.downloaddata();
        this.renderChart();
    }
...
async clearSearchQuery() {
    ...
    sessionStorage.removeItem("hr_dashboard_search");
    await this.downloaddata();
    this.renderChart();
}
...
async toggleFilter(filterName) {
    ...
    sessionStorage.setItem("hr_dashboard_active_filters", JSON.stringify(this.state.active_filters));
    await this.downloaddata();
    this.renderChart();
}
...
async toggleGroupBy(groupName) {
    ...
    sessionStorage.setItem("hr_dashboard_group_by", JSON.stringify(this.state.group_by_list));
    await this.downloaddata();
    this.renderChart();
}
...
async downloadPdf() {
    ...
    doc.autoTable({
        ...
        headStyles: {fillColor: [23, 162, 184], fontSize: 12 },
            ...
            data: {
                ...
                datasets: [
                    ...
                    },
                    {
                        ...
                        borderWidth: 3, pointRadius: 5, pointBackgroundColor: '#fff',
