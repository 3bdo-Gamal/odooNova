...
#filters
        is_active = active_filters.get('active_emp', True)
        is_archived = active_filters.get('archived_emp', False)
        active_state = None
        valid_user_ids = []
        if is_active and not is_archived:
            active_state = True
            employee_domain.append(('active', '=', True))
        elif is_archived and not is_active:
            active_state = False
            employee_domain.append(('active', '=', False))
        if active_state is not None:
            domain_attendance.append(('employee_id.active', '=', active_state))
            domain_leaves.append(('employee_id.active', '=', active_state))
            state_emps = self.env['hr.employee'].with_context(active_test=False).search([('active', '=', active_state)])
            valid_user_ids = state_emps.mapped('user_id').ids if state_emps.mapped('user_id') else [-1]
            domain_tasks_complete.append(('user_ids', 'in', valid_user_ids))
            domain_tasks_not_complete.append(('user_ids', 'in', valid_user_ids))
            domain_sales.append(('user_id', 'in', valid_user_ids))
        if search:
            employee_domain.append(('name', 'ilike', search))
            domain_attendance.append(('employee_id.name', 'ilike', search))
            domain_leaves.append(('employee_id.name', 'ilike', search))
            domain_tasks_complete.append(('user_ids.name', 'ilike', search))
            domain_sales.append(('user_id.name', 'ilike', search))
            domain_tasks_not_complete.append(('user_ids.name', 'ilike', search))
            domain_emp_start.append(('name', 'ilike', search))
            domain_emp_end.append(('name', 'ilike', search))
            domain_emp_left.append(('name', 'ilike', search))
#dep_filter
        if department_id:
        ...
        total_leave_days = float(leaves[0].get('number_of_days') or 0) if leaves else 0
        ...
        # kpi average_leaves
        if (employee_count > 0):
            average_leaves = float((total_leave_days / total_expected_company_days) * 100)
        else:
            average_leaves = 0
        ...
        attendance = min(round(total_percentage, 2), 100)
        # kpi absence
        absence_days = max(0, net_expected_days - actual_attendance_days)
        absence = (absence_days / net_expected_days) * 100 if net_expected_days > 0 else 0
        absence = round(min(absence, 100), 2)
        workload_data = None
        workload_hours = 0
        # kpi tasks complete
        ...
        # kpi productivity
        production_kpi = (tasks_complete / tasks_all) * 100 if tasks_all else 0
        # kpi turnover
        ...
        emp_turnover = round((emp_left / avg_employees) * 100, 2) if avg_employees > 0 else 0
        # kpi sales/emp
        ...
        for task in tasks_not_completed:
            ...
            if not users:
                ...
                if not department_id:
                    ...
                    chart_dict[label] = chart_dict.get(label, 0.0) + hours
            else:
                t_share = hours / len(users)
                for user in users:
                    if search and search.lower() not in user.name.lower():
                        continue
                    if department_id and user.id not in dept_user_ids:
                        continue
                    if active_state is not None and user.id not in valid_user_ids:
                        continue
                    ...
                    if group_by_list:
                        ...
                        for gb in group_by_list:
                            if gb == 'department':
                                label_parts.append(
                                    emp.department_id.name if emp and emp.department_id else "No Department")
                            elif gb == 'manager':
                                label_parts.append(emp.parent_id.name if emp and emp.parent_id else "No Manager")
                            elif gb == 'job_position':
                                label_parts.append(emp.job_id.name if emp and emp.job_id else "No Position")

                        label = " / ".join(label_parts) if label_parts else user.name
                    else:
                        label = user.name

                    chart_dict[label] = chart_dict.get(label, 0) + t_share
        ...
        return {
            ...
            'absence': absence,
            "average_leaves": round(average_leaves, 2),
